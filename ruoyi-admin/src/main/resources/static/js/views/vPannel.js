...
function init_ec_edulevel() {
    var bt_mbygroup = echarts.init(document.getElementById('ec_edulevel'));
    $.ajax({
        type: "GET",
        url: "/api/count/b_edulevel",
        datatype: "JSON",
        success: function (data) {
            ...
            for (var i = 0; i < btygroup_data.length; i++) {
                if (btygroup_data[i].edulevel == '') {
                    x_data.push('未知');
                } else {
                    y_data.push(parseInt(btygroup_data[i].pid));
                    x_data.push(btygroup_data[i].edulevel);
                }
            }
            /*console.log(x_data);*/
            /*console.log(y_data);*/
